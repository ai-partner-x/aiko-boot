          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50/80">
                ...
                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-600">数量</th>
                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-600">单价</th>
                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-600">净金额</th>
                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-600">税额</th>
                <th className="px-6 py-3 text-center text-xs font-semibold text-gray-600">状态</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data.items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right text-sm text-gray-900">{formatAmount(item.unitPrice)}</td>
                  <td className="px-6 py-4 text-right text-sm font-medium text-gray-900">{formatAmount(item.netAmount)}</td>
                  <td className="px-6 py-4 text-right text-sm text-gray-500">{formatAmount(item.taxAmount)}</td>
                  <td className="px-6 py-4 text-center">
                    <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-medium">
                      {item.status === 'open' ? '待收货' : '已收货'}
                    </span>
                  </td>
                </tr>
              ))}
            ...
            <tfoot>
              <tr className="bg-gray-50">
                <td colSpan={5} className="px-6 py-4 text-right text-sm font-medium text-gray-500">合计</td>
                <td className="px-6 py-4 text-right text-sm font-bold text-gray-900">{formatAmount(data.netAmount)}</td>
                <td className="px-6 py-4 text-right text-sm font-bold text-gray-900">{formatAmount(data.taxAmount)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
