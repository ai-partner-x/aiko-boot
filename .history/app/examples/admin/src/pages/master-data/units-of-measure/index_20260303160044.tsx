...
export function UnitsOfMeasurePage() {
  ...
  const renderDetail = (uom: UnitOfMeasure) => (
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center text-xl font-bold">{uom.uomCode}</div>
          <div><h2 className="text-xl font-bold text-gray-900">{uom.uomName}</h2><p className="text-sm text-gray-500 mt-1">{uom.uomCode} · {uom.dimension}</p></div>
          <div className="ml-auto text-right"><div className="text-2xl font-bold text-gray-900">{uom.conversionFactor}</div><div className="text-sm text-gray-500">换算系数 (对{uom.baseUnit})</div></div>
        </div>
      </div>
