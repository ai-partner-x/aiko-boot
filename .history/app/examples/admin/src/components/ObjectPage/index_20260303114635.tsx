        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50">
          <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-end gap-3">
            {footerActions.map((action, index) => {
              const isLast = index === footerActions.length - 1;
              const isPrimary = action.variant === 'primary' || (!action.variant && isLast);
              const isSecondary = action.variant === 'secondary' || (!action.variant && !isLast);
              ...
              return (
                <div key={action.key} className="flex items-center">
                  <button
                    ...
                    className={cn(
                      ...
                      isPrimary
                        ...
                        : isSecondary
                        ? 'bg-transparent hover:bg-gray-100 text-gray-700 border border-gray-300 rounded-lg'
                        : action.variant === 'danger'
                        ? 'bg-red-600 hover:bg-red-700 text-white rounded-lg'
                        : 'hover:bg-gray-100 text-gray-600 rounded-lg',
                      isPrimary && 'rounded-r-none'
                    )}
                  >
                    {action.loading ? (
                  ...
                  {isPrimary && (
                    <button
                      className="h-10 px-2 bg-blue-600 hover:bg-blue-700 text-white border-l border-blue-500 rounded-r-lg flex items-center"
                      onClick={action.onClick}
                    >
...
function SectionCard({ section, mode, compact }: SectionCardProps) {
  return (
    <div
      ...
      className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden scroll-mt-32"
    >
      <div
        className={cn(
      ...
      <div className={compact ? 'p-4' : 'p-6'}>{section.content}</div>
    </div>
  );
}
