...
export function MasterDetail<T extends MasterDetailItem>({
  ...
  return (
    <div className="h-[calc(100vh-56px)] flex flex-col bg-gray-50 overflow-hidden -m-6 w-[calc(100%+48px)]">
      {/* 页面头部 */}
        ...
        <div className="flex-1 flex flex-col bg-gray-50">
          ...
          <div className="flex-1 overflow-y-auto">
            {editMode === 'create' && renderForm ? (
              <div className="p-6">{renderForm(null, 'create')}</div>
            ) : editMode === 'edit' && selectedItem && renderForm ? (
              <div className="p-6">{renderForm(selectedItem, 'edit')}</div>
            ) : selectedItem ? (
              <>
                {/* 查看模式时在内容顶部显示操作按钮 */}
                {(allowEdit || allowDelete) && (
                  <div className="px-6 pt-6 pb-0 flex justify-end gap-2">
                    {allowEdit && renderForm && (
                      <button
                        onClick={handleEdit}
                        className="h-9 px-4 rounded-lg bg-blue-50 text-blue-600 text-sm font-medium hover:bg-blue-100 flex items-center gap-2 transition-colors"
                      >
                        {Icons.edit}
                        <span>编辑</span>
                      </button>
                    ...
                    {allowDelete && onDelete && (
                      <button
                        ...
                        className="w-9 h-9 rounded-lg bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-600 flex items-center justify-center transition-colors"
                        title="删除"
                      >
                        {Icons.delete}
                      </button>
                ...
                <div className={cn("px-6 pb-6", !(allowEdit || allowDelete) && "pt-6")}>
