...
import { createApiClientFromMeta, UserController, UserControllerMeta } from '@user-crud/api/client';
...
import {
  ...
  TableHeader,
  TableRow,
} from '@user-crud/admin-component';

// Feign 风格 API 客户端
const userApi = createApiClientFromMeta(UserControllerMeta, UserController, {
...
export default function AdminPage() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<User[]>('users', () => userApi.list(), {
  ...
  const { trigger: createUser, isMutating: isCreating } = useSWRMutation(
    'users',
    (_key: string, { arg }: { arg: CreateUserDto }) => userApi.create(arg),
    {
      onSuccess: () => {
        toast.success('创建成功');
        mutate();
  ...
  const { trigger: updateUser, isMutating: isUpdating } = useSWRMutation(
    'users',
    (_key: string, { arg }: { arg: { id: string; data: UpdateUserDto } }) =>
      userApi.update(arg.id, arg.data),
    {
      onSuccess: () => {
        toast.success('更新成功');
        mutate();
  ...
  const { trigger: deleteUser } = useSWRMutation(
    'users',
    (_key: string, { arg }: { arg: string }) => userApi.delete(arg),
    {
      onSuccess: () => {
        toast.success('删除成功');
        mutate();
  ...
  const onSubmit = async (data: UserFormData) => {
    if (editingUser) {
      await updateUser({ id: String(editingUser.id), data });
    } else {
    ...
    <div className="container mx-auto p-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>用户管理 (Admin)</CardTitle>
